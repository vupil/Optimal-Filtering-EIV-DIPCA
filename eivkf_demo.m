...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The readme.txt file shows how the functions eivkf, dipca_eivkf, and dipca could
% be used. Please run the following commands to get their respective documentations in Matlab: 
% 1) doc eivkf_ref
% 2) help eivkf_ref
% 3) doc dipca_eivkf_ref
% 4) help dipca_eivkf_ref
...
% noise-free output
% LTI system: y[k] + 0.4y[k-1] + 0.6y[k-5] = 1.4u[k-1] + 1.8u[k-2]
...
% noise-free output
% LTI system: y[k] + 0.2y[k-1] + 0.6y[k-5] = 1.2u[k-1] + 1.6u[k-2]
...
%% Residual Generation using EIV-KF assuming known process order and parameters (no DIPCA)
% % system 1
% out_par = [1 0.4 0.6];
% inp_par = [0 1.4 1.8];

% system 2
out_par = [1 0.2 0 0 0 0.6];
inp_par = [0 1.2 1.6 0 0 0];

noise_var = var(n);

[residuals, y_filtered, u_filtered, noise_var,eigval_f,out_par,inp_par] = eivkf(zn,out_par,inp_par, noise_var, true)

%% Estimation of the EIV system and Residual Generation using EIV-KF (with DIPCA)
